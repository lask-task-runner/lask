...
Each directory here is a whole Lask project — a `main.lask` you can check, run and edit in place. The files are written to be read: the commentary is in the code, next to the thing it is about, and the commands below are the ones worth running while you read.
...
**Pure.** Bindings and annotations, type aliases, records against maps, unions and optional fields, `case` type dispatch, `cast`, and a generic function.
...
The thing to take away: `a?: T` and `a: T | Null` answer different questions, and neither is ever inferred — each one is something you wrote.
...
**Pure.** `do` blocks, `if` as an expression, the guard `return`, all three forms of `case`, and `for`.
...
The thing to take away: `async` starts the work, `await` joins it, and a failure inside a handle is raised where it is awaited.
...
**Pure.** Numbers, strings, POSIX regular expressions, arrays, maps, and the two ways the library reports absence.
...
The thing to take away: there is no overloading. One name, one signature — which is why `size` is for arrays and a map's size is `size(keys(m))`.

## 11-data

**Pure.** `from_json` and `to_json`, `encode` / `decode` for YAML, TOML, CSV and dotenv, `cast`, `sha256`, `md5` and base64.
...
The thing to take away: every format decodes to the same handful of value kinds, so `cast` is the same step whichever syntax the data arrived in.
...
The thing to take away: every filesystem call names the environment it acts in, as its last argument. There is no default and no host by accident.

## 14-docs-and-cli

**Pure.** Doc comments end to end — summary, description, `@param`, `@return`, `@example`, `@complete` in all four forms, `@hidden` — and what the CLI does with a signature.
...
The thing to take away: `-` maps to `_`, so `publish(--dry_run = false)` is `--dry-run` on the command line, and the comment above a task is the only place its help is written.
