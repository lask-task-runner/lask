...
Records have a statically fixed field set and an absent optional key reads as null, so `.f` never fails at runtime; maps are for keys that vary, and `m[k]` fails on a missing key (`get_or` does not). → [6.8](spec.md#68-accessor-expressions)
...
→ [spec 6.6](spec.md#66-command-execution-expressions), [6.7](spec.md#67-environment-expressions), [ch. 10](spec.md#10-execution-environments)
...
`run` writes nothing to stdout, `eval` writes the return value there, and logs and diagnostics always go to stderr. That is what makes `lask eval a | lask run b` work. → [11.3](spec.md#113-inputoutput-contract)
...
→ [spec ch. 15](spec.md#15-built-in-library). No overloading: one name, one signature.
