...
- The environment expression must be statically resolvable: an environment expression (6.7), or an identifier resolving to a top-level value declaration whose right-hand side is, transitively, an environment expression. Any other expression is a static error (`E-TYPE-COMMAND-DECL`). The restriction makes every declared environment enumerable (11.4), pinnable (10.3), and resolvable by `cmd` (11.8) without evaluating the module.
